...
class Facebook_Login_Public {
	...
	public function print_button() {
		$temporal_redirect = get_field('temporal_redirect', 'options')->ID;

		$redirect = get_permalink($temporal_redirect);
					//"http://waremill.theclient.org/customer-dasboard/"; 
					//apply_filters( 'flp/redirect_url', ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
		...
		echo apply_filters('fbl/login_button', '<a href="#" class="css-fbl js-fbl" data-redirect="'.$redirect.'" data-fb_nonce="' . wp_create_nonce( 'facebook-nonce' ).'"><i class="fa fa-facebook"></i>'. __('Connect with Facebook', 'fbl') .'<img data-no-lazy="1" src="'.site_url('/wp-includes/js/mediaelement/loading.gif').'" alt="" style="display:none"/></a>');
	}
	...
	public function login_or_register_user() {
		...
		if ( $user_obj ){
			...
			/**********************************************************************************************/
			$user['user_login'] = apply_filters( 'fbl/generateUsername', $user['user_email'] );
			//OLD: $user['user_login'] = apply_filters( 'fbl/generateUsername', $this->generateUsername( $fb_user ) );
			...
			if( !is_wp_error( $user_id ) ) {
				...
				/*************************************************************************/
				// new code to change default role
				wp_update_user( array( 
						'ID' => $user_id, 
						'role' => 'customer' ,
					 	'user_url' => "http://facebook.com/profile.php?id=".$user['fb_user_id']
				) );
